(() => {
  ...
  function setNotice(message, kind = 'success') { let host = document.getElementById('mpa-toast'); if (!host) { host = document.createElement('div'); host.id = 'mpa-toast'; host.className = 'mpa-toast'; host.setAttribute('role', 'alertdialog'); host.setAttribute('aria-live', 'polite'); document.body.append(host); } const title = kind === 'error' ? 'ยังดำเนินการต่อไม่ได้' : kind === 'warning' ? 'กรุณาตรวจสอบข้อมูล' : kind === 'welcome' ? 'ยินดีต้อนรับกลับมา' : 'ดำเนินการสำเร็จ'; const duration = kind === 'welcome' ? 2000 : 15000; clearTimeout(setNotice.timer); clearInterval(setNotice.countdown); host.className = `mpa-toast ${kind}`; host.hidden = false; host.style.animation = 'none'; void host.offsetWidth; host.style.animation = ''; host.innerHTML = `<div class="mpa-toast-icon" aria-hidden="true">${kind === 'error' ? '!' : kind === 'warning' ? '?' : kind === 'welcome' ? '✓' : '✓'}</div><div class="mpa-toast-body"><strong>${escapeHtml(title)}</strong><p>${escapeHtml(message)}</p><button type="button" class="mpa-toast-ok">ตกลง</button></div><small class="mpa-toast-countdown" aria-label="เวลาที่เหลือ"></small>`; const close = () => { clearTimeout(setNotice.timer); clearInterval(setNotice.countdown); if (host.hidden || host.classList.contains('is-leaving')) return; host.classList.add('is-leaving'); setTimeout(() => { host.hidden = true; host.classList.remove('is-leaving'); }, 180); }; host.querySelector('.mpa-toast-ok').onclick = close; let remaining = Math.ceil(duration / 1000); const countdown = host.querySelector('.mpa-toast-countdown'); countdown.textContent = `${remaining}s`; setNotice.countdown = setInterval(() => { remaining -= 1; countdown.textContent = `${Math.max(0, remaining)}s`; if (remaining <= 0) clearInterval(setNotice.countdown); }, 1000); setNotice.timer = setTimeout(close, duration); }
