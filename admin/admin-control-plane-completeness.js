(() => {
  ...
  const openRefundAction = async (row, afterSave) => {
    ...
    const content = `${action === 'approve' ? `<label class="mpa-field"><span>${amountLabel}</span><input name="amount" type="number" min="0.01" max="${esc(row.requested_amount)}" step="0.01" value="${esc(defaultAmount)}" required></label><label class="mpa-field"><span>ผลพิจารณา</span><select name="decision"><option value="approve">อนุมัติคำขอคืนเงิน</option><option value="reject">ปฏิเสธคำขอคืนเงิน</option></select></label>` : `<label class="mpa-field"><span>${amountLabel}</span><input name="amount" type="number" min="0.01" max="${esc(row.approved_amount)}" step="0.01" value="${esc(defaultAmount)}" required></label><label class="mpa-field"><span>เลขอ้างอิงการโอน</span><input name="reference" maxlength="120" placeholder="เช่น TXN-20260820-001"></label><label class="mpa-field"><span>หลักฐานการโอนคืน (ถ้ามี)</span><input name="proof" type="file" accept="image/jpeg,image/png,image/webp"><small class="mpa-muted">ต้องมีเลขอ้างอิงหรือหลักฐานอย่างน้อยหนึ่งอย่าง</small></label>`}<label class="mpa-field admin-form-full"><span>เหตุผล / หมายเหตุ</span><textarea name="reason" rows="3" minlength="3" maxlength="1000" required placeholder="ระบุข้อมูลที่ตรวจสอบและเหตุผลอย่างน้อย 3 ตัวอักษร">${action === 'approve' ? esc(row.reason || '') : ''}</textarea></label><dl class="admin-withdrawal-review-grid"><div><dt>ออร์เดอร์</dt><dd>${esc(row.order_id)}</dd></div><div><dt>ลูกค้า</dt><dd>${esc(row.order.customer_name || row.customer_id || '-')}</dd></div><div><dt>ร้านค้า</dt><dd>${esc(row.order.store_name || '-')}</dd></div><div><dt>สถานะคำขอ</dt><dd>${esc(refundLabel(row.status))}</dd></div><div><dt>ยอดที่ขอ</dt><dd>${refundMoney(row.requested_amount)}</dd></div>${row.approved_amount !== null && row.approved_amount !== undefined ? `<div><dt>ยอดที่อนุมัติ</dt><dd>${refundMoney(row.approved_amount)}</dd></div>` : ''}</dl>`;
